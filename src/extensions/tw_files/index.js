...
const showFilePrompt = (accept, as) => new Promise((_resolve) => {
    ...
    const readFile = (file) => {
        ...
        reader.onload = () => {
            if (isReadingAsBuffer) {
                callback(/** @type {string} */(JSON.stringify(BufferStuff.bufferToArray(reader.result))));
                return;
            }
        ...
        } else if (as === AS_BUFFER) {
            isReadingAsBuffer = true;
...
const download = (text, file, encoding) => {
    let _blob;
    if (encoding === AS_DATA_URL) {
        _blob = dataURLtoBlob(text);
    } else if (encoding === AS_BUFFER) {
        const array = validateArray(text);
        if (array.isValid) {
            text = BufferStuff.arrayToBuffer(array.array);
        }
        _blob = new Blob([text]);
    } else {
        _blob = new Blob([text]);
...
class Files {
    ...
    download(args) {
        download(args.text, args.file, args.encoding);
    }
