...
const parseColor = (color, callback) => {
    ...
    if (match) {
        const url = match[1];
        return vm.securityManager.canFetch(url).then((allowed) => {
            if (allowed) {
                callback(color);
            }
        });
...
class MonitorStyles {
    ...
    setvarpos(args, util) {
        setVariableMonitorPosition(
            ...
            args.NAME,
            "",
            Cast.toNumber(args.X) + this.runtime.stageWidth / 2,
            this.runtime.stageHeight / 2 - Cast.toNumber(args.Y)
        );
    }
    ...
    setAskURI(args) {
        return this.runtime.vm.securityManager.canFetch(args.URL).then((allowed) => {
            if (allowed) {
